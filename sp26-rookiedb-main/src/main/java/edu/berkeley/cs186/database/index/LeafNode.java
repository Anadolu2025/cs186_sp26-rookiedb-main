...
class LeafNode extends BPlusNode {
     ...
     * to that page.很有意思的现象！两个constructor函数！
     */
    ...
    private LeafNode(BPlusTreeMetadata metadata, BufferManager bufferManager, Page page,
        ...
        try {
            ...
            assert (keys.size() <= 2 * metadata.getOrder());//oder is the d!
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        // TODO(proj2): implement
        if (keys.contains(key)) {//检查有没有重复的
            throw new BPlusTreeException("duplicate key");
        }
        int index=InnerNode.numLessThan(key,keys);//这个我是第一次看到，调用另一个类里面的函数，我刚才还在这个file里面找numless在哪里呢！
        keys.add(index,key);
        rids.add(index,rid);
        int n= keys.size();
        if(n<=2* metadata.getOrder()){
            sync();//这个很重要，把keys和rids写入disk
            return Optional.empty();
        }
        else {//其实overflow的情况只有d和d+1，所以只要把数组拆开就可以了，再建立新的页
            ...
            Optional<Long> oldRightSibling = rightSibling;

            LeafNode rightleaf=new LeafNode(metadata, bufferManager, rightKeys,
                    rightRids, oldRightSibling, treeContext);//创建的新的叶子结点
            //更新原来的结点
            keys=leftKeys;
            rids=leftRids;
            ...
            return Optional.of(new Pair<>(splitkey, rightleaf.getPage().getPageNum()));
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
            float fillFactor) {
        // TODO(proj2): implement 6.26
        float targetsize=2* metadata.getOrder()*fillFactor;
        ...
        rightKeys.add(newpair.getFirst());
        rightRids.add(newpair.getSecond());


        Optional<Long> oldRightSibling = rightSibling;//新的leafnode的右边的兄弟是原来结点右边的兄弟
        LeafNode rightleaf=new LeafNode(metadata, bufferManager, rightKeys,
                rightRids, oldRightSibling, treeContext);//创建的新的叶子结点
        rightSibling = Optional.of(rightleaf.getPage().getPageNum());

        sync();
        return Optional.of(new Pair<>(rightKeys.get(0), rightleaf.getPage().getPageNum()));//其实大部分都是照抄上面的put代码
    ...
    public byte[] toBytes() {
        ...
        int isLeafSize = 1;
        int siblingSize = Long.BYTES;//一个long，有多少bytes
        int lenSize = Integer.BYTES;
        int keySize = metadata.getKeySchema().getSizeInBytes();
        int ridSize = RecordId.getSizeInBytes();
        int entriesSize = (keySize + ridSize) * keys.size();
        int size = isLeafSize + siblingSize + lenSize + entriesSize;//how big is a page

        ByteBuffer buf = ByteBuffer.allocate(size);//这里也有分配地址真的没有绷住
        buf.put((byte) 1);
    ...
    public static LeafNode fromBytes(BPlusTreeMetadata metadata, BufferManager bufferManager,
        ...
        byte nodeType = buf.get();//第一次从buffer读取数据
        assert(nodeType == (byte) 1);//我一开始照抄的inner，导致出现assertionerror！

        long sibling=buf.getLong();
        //sibling的pageid是long类型的，第二次读
        Optional<Long> rightSibling = sibling == -1L
                ? Optional.empty()
                : Optional.of(sibling);
        ...
        int n = buf.getInt();//第三次从buffer读取数据
